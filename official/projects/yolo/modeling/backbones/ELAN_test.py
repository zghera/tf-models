...
from official.projects.yolo.modeling.backbones import ELAN


class ElanTest(parameterized.TestCase, tf.test.TestCase):

  @parameterized.parameters((256, 'elan_regular', 1, 1, False))
  def test_network_creation(self, input_size, model_id, endpoint_filter_scale,
                            scale_final, dilate):
    """Test creation of ResNet family models."""
    tf.keras.backend.set_image_data_format('channels_last')

    network = ELAN.ELAN(model_id=model_id,
                           min_level=3,
                           max_level=5,
                           dilate=dilate)
  ...
  def test_sync_bn_multiple_devices(self, strategy, use_sync_bn):
    ...
    with strategy.scope():
      network = ELAN.ELAN(model_id='elan_regular', min_size=3, max_size=5)
      _ = network(inputs)

  # TODO(ibrahim): Add test_input_specs.
  ...
  def test_input_specs(self, input_dim):
    ...
    network = ELAN.ELAN(model_id='elan_regular',
                              min_level=3,
                              max_level=5,
  ...
  def test_serialize_deserialize(self):
    ...
    network = ELAN.ELAN(**kwargs)

    expected_config = dict(kwargs)
    self.assertEqual(network.get_config(), expected_config)

    # Create another network object from the first object's config.
    new_network = ELAN.ELAN.from_config(network.get_config())
