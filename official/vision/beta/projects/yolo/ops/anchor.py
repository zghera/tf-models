...
class YoloAnchorLabeler:
  ...
  def _get_anchor_free(self,
    ...
    grid_points = gen(height, width, 1, boxes.dtype)[0]
    ...
    (
        ...
        indexes,
    ) = self.dynamic_k_matching(box_delta, classes, len(classes), is_in_index)
  ...
  def dynamic_k_matching(pair_wise_ious, gt_classes, num_gt, fg_mask):
    """Builds the labels for a single image, not functional in batch mode. 
    
    Args: 
      cost: `Tensor` of shape [None, 4] indicating the object locations in 
        an image. 
      pair_wise_ious: `Tensor` of shape [None] indicating the each objects classes.
      gt_classes: `int` for the images width. 
      num_gt: `int` for the images height.
      fg_mask: `int` for the maximum number of expanded boxes to allow. 

    Returns: 
      num_fg: `Tensor` of shape [None, 3] of indexes in the final grid where 
        boxes are located.  
      gt_matched_classes: `Tensor` of shape [None, 8] the value to place in the final grid.
      pred_ious_this_matching: `Tensor` of [width/stride, height/stride, num_anchors, 1] holding 
        a mask of where boxes are locates for confidence losses. 
      matched_gt_inds: 
    """    
